...
def get_args(arguments=None, parse_help=True):
    ...
    parser.add_argument(
        '--setup-test',
        help="Tests to be run in order to set up the environment.  They "
            "are not run interactively, and will not be saved to the file "
            "generated by 'Save test'.  This argument can be supplied "
            "multiple times to run more than one test as setup.",
        action="append")
    parser.add_argument(
        '--verbose',
        help="Print out LOTS of comments into the console telling what xpaths are being used to find the nouns.",
        action="store_true",
        default=False)
